...
- **FR-005**: Al recibir la notificación WebSocket con `status: "ready"`, el frontend DEBE llamar a `GET /v1/jobs/:jobId` para obtener `downloadUrl` y poblar el estado de la aplicación con `jobId`, `title`, `duration` (como `durationSeconds`), `thumbnailUrl` y `downloadUrl`. El WS payload no incluye `downloadUrl` directamente; la URL pre-firmada se obtiene siempre mediante la llamada HTTP posterior.
...
- **FR-008**: El botón "Descargar MP3" en `PreviewScreen` DEBE llamar a `POST /v1/jobs/:jobId/mp3` sobre el job activo (sin crear un nuevo job) y descargar el resultado al recibir la notificación WebSocket con `status: "mp3_ready"`.
- **FR-009**: Los botones de descarga de segmentos en `TrimScreen` DEBEN llamar a `POST /v1/jobs/:jobId/trim` con `trimStart` y `trimEnd` (o `POST /v1/jobs/:jobId/mp3` para MP3) e iniciar la descarga al recibir la notificación WebSocket con `status: "trimmed"` o `status: "mp3_ready"` respectivamente.
