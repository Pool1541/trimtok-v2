...
import { CreateGifUseCase } from "../../processing/application/create-gif.usecase.js";
...
const logger = createLogger("workers/gif-worker");
...
export const handler: SQSHandler = async (event): Promise<SQSBatchResponse> => {
  ...
  for (const record of event.Records) {
    await withSqsRecordContext(record, "workers/gif-worker", async () => {
      ...
      const useCase = new CreateGifUseCase(
        jobRepo,
      ...
      try {
        logger.debug("gif_started");
        await writeEvent(msg.jobId, "gif_started");
        await useCase.execute(msg.jobId, msg.videoId, msg.s3Key, msg.trimStart, msg.trimEnd);
        await writeEvent(msg.jobId, "gif_completed");
        logger.info("gif completed");
      } catch (err) {
        logger.error({ err }, "error processing gif");
        if (job.retryCount < 1) {
          ...
          await writeEvent(msg.jobId, "gif_failed").catch((e) => logger.error({ err: e }, "failed to write gif_failed event"));
