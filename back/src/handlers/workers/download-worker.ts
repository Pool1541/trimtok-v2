...
export const handler: SQSHandler = async (event): Promise<SQSBatchResponse> => {
  ...
  for (const record of event.Records) {
    ...
    try {
      ...
      console.error("[download-worker] Failed to parse SQS message body:", record.body, parseErr);
      continue;
    }

    console.log("[download-worker] Processing message:", { jobId: msg.jobId, tiktokUrl: msg.tiktokUrl, format: msg.format });
    ...
    try {
      console.log("[download-worker] Fetching job from DB:", msg.jobId);
      const job = await jobRepo.findById(msg.jobId);
      if (!job) {
        console.warn("[download-worker] Job not found in DB, skipping:", msg.jobId);
        continue;
      }
      console.log("[download-worker] Job found:", { status: job.status, retryCount: job.retryCount, videoId: job.videoId });

      const videoId = job.videoId ?? msg.jobId;
      console.log("[download-worker] Writing download_started event");
      await writeEvent(msg.jobId, "download_started");

      console.log("[download-worker] Executing DownloadVideoUseCase");
      await useCase.execute(msg.jobId, msg.tiktokUrl, videoId, msg.format);
      console.log("[download-worker] DownloadVideoUseCase completed successfully");

      await writeEvent(msg.jobId, "download_completed");
      console.log("[download-worker] Notifying subscribers");
      await notifier.execute(msg.jobId);
      console.log("[download-worker] Job finished successfully:", msg.jobId);
    } catch (err) {
      console.error("[download-worker] Error processing job:", msg.jobId, err instanceof Error ? { message: err.message, stack: err.stack } : String(err));
      const job = await new JobDynamoRepo().findById(msg.jobId).catch(() => null);
      if (job && job.retryCount < 1) {
        console.warn("[download-worker] Scheduling retry (retryCount:", job.retryCount, ") for job:", msg.jobId);
        await new JobDynamoRepo().updateStatus(msg.jobId, job.status, {
          retryCount: job.retryCount + 1,
        });
        batchItemFailures.push({ itemIdentifier: record.messageId });
      } else {
        console.error("[download-worker] Max retries reached, marking job as error:", msg.jobId);
        await new JobDynamoRepo().updateStatus(msg.jobId, JobStatus.error, {
          errorMessage: err instanceof Error ? err.message : String(err),
        }).catch((e) => console.error("[download-worker] Failed to update job status to error:", e));
        await writeEvent(msg.jobId, "download_failed").catch((e) => console.error("[download-worker] Failed to write download_failed event:", e));
        await new NotifyJobUpdateUseCase(
          new ConnectionDynamoRepo(),
          new WebSocketAdapter(),
          new JobDynamoRepo(),
        ).execute(msg.jobId).catch((e) => console.error("[download-worker] Failed to send error notification:", e));
      }
    }
  }
